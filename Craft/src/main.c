...
void handle_movement(double dt) {
	...
	/* TODO: Comment out gflwGetKey code for efficiency */
        if (glfwGetKey(g->window, CRAFT_KEY_FORWARD) || go_forward) sz--;
        if (glfwGetKey(g->window, CRAFT_KEY_BACKWARD) || go_backward) sz++;
        if (glfwGetKey(g->window, CRAFT_KEY_LEFT) || go_left) sx--;
        if (glfwGetKey(g->window, CRAFT_KEY_RIGHT) || go_right) sx++;
...
int main(int argc, char **argv) {
    ...
    while (running) {
        ...
        while (1) {
			...
			// CHANGE
			int PIXEL_BYTES = 1;
			unsigned char* screen_shot = (unsigned char* )malloc(WINDOW_WIDTH * WINDOW_HEIGHT * PIXEL_BYTES);
			glReadPixels(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, GL_LUMINANCE, GL_UNSIGNED_BYTE, screen_shot);

			for(int i = 0; i < WINDOW_WIDTH * WINDOW_HEIGHT; ++i) {
				printf("%d ", screen_shot[i]);
			}
			printf("\n DONE \n");
			// CvMat mat = cvMat(WINDOW_HEIGHT, WINDOW_WIDTH, CV_8UC1);
			// cvSaveImage("test.png", screen_shot);

			/* FILE *f = fopen("test.txt", "wb"); */
			/* // fwrite(screen_shot, sizeof(unsigned char), sizeof(screen_shot), f); */
			/* for(int i=0; i<WINDOW_WIDTH * WINDOW_HEIGHT; ++i) */
			/* 	scanf("%d",&screen_shot[i]); */
			/* fwrite(screen_shot, sizeof(screen_shot) , 1, f); */
			/* fclose(f); */
